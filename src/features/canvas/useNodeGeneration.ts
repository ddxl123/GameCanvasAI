 ...
 * - generate(element, prompt, context?)：调用 aiService（已配置 key 走真实 API，否则模拟）
...
export function useNodeGeneration() {
  ...
  const generate = useCallback(
    (element: CanvasElement, prompt: string, context?: GenerationContext) => {
      // 前置去重：已在生成中则不重复发起
      ...
      return generateContent(element, prompt, context).finally(() => {
        setGeneratingKeys((prev) => {
          const next = new Set(prev);
