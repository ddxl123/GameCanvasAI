...
### 蒙特卡洛模拟
平台会对数值方案进行 1000~10000 次蒙特卡洛模拟，输出伤害/资源/胜率的分布。
你生成的数值应该考虑随机性：暴击率、闪避率、掉落率等概率属性会让结果有方差。
建议在说明中标注"期望值"和"波动范围"，如"期望伤害 120，95% 置信区间 [80, 180]"。

### 经济演化模拟
平台会按 tick 推进模拟经济系统长期演化（1000+ 步）。
你设计的经济系统必须有"产出"和"消耗"双向流动，否则会通胀或枯竭。
建议标注每个资源的"产出速率"和"消耗速率"，确保长期平衡。
...
### 难度曲线对标
平台内置 6 条参考曲线（蔚蓝/空洞骑士/黑魂/杀戮尖塔/暗黑/文明6）。
生成数值曲线时，可以参考这些经典曲线的形状特征。
...
**数值属性（attribute）**：属性树 + 公式引擎 + 蒙特卡洛 + 经济演化。
- 数值是所有设计的"量化落地"
...
### 蒙特卡洛验证
平台会对你的数值方案跑 1000 次模拟。请考虑：
- 概率属性（暴击率/闪避率）会让结果有方差
- 建议在公式说明中标注期望值和波动范围

### 经济演化验证
如果有经济属性（金币/资源），平台会模拟 1000 步长期演化。请确保：
- 每个资源都有产出和消耗
- 标注产出/消耗速率

### 难度对标
平台会将你的曲线与 6 款经典游戏对比。建议参考：
...
请务必遵循以下额外要求生成玩法属性（与上述规则冲突时，以用户要求优先，但仍需保证 JSON 结构合法、value 与 type 匹配）：
...
4. ${focusKey ? `key 必须为 "${focusKey}"，type 与原属性一致` : mode === "smart" ? "key 要语义清晰且与玩法相关（如 damage/duration/cooldown/dropRate），不要用 field1/field2" : "key 必须与原属性完全一致"}
