...
## 实时工具调用（Agentic 模式）
你可以**连续多次通过 tool_calls 调用工具**，每次工具调用的结果会立即应用到画布并回传给你，你可以基于更新后的状态继续下一步操作。这意味着：
...
参数为一个对象，包含两个字段：
- nodes: 数组，每项 { id: string(必填), label: string(必填), type: string(必填, 40种节点类型之一，如 attack/condition/reward/resource/state/event/penalty/feedback 等), description: string, customFields?: [{ key: string, type: "number"|"string"|"boolean"|"select"|"range"|"color"|"reference", value: any }] }
- edges: 数组，每项 { source: string(节点 id), target: string(节点 id), type: string(17种边类型之一，如 reference/cause/trigger/feedback 等), label?: string }

## 参考示例（ARPG 战斗循环片段）
节点：[attack(攻击:造成伤害), enemy(敌人:受击目标), reward(奖励:掉落)]
边：attack → enemy (cause), enemy → reward (trigger)
`,
    },
  ];
}
...
## 增量工具参数 schema
- update_node: { nodeLabel: string(必填, 必须与现有节点 label 完全一致), updates: { label?: string, description?: string, type?: string, customFields?: array } }
- remove_node: { nodeLabel: string(必填) }
- add_node_to_existing: { nodes: [{ label: string(必填), type: string(必填), description?: string }], edges: [{ sourceLabel: string(必填), targetLabel: string(必填), type: string(必填), label?: string }] }
- patch_formula: { attributeName: string(必填), expression?: string, description?: string }
`,
    },
  ];
}
...
## apply_numeric 工具参数 schema
- attributes: 数组，每项 { name: string(必填), type: "number"|"string"|"boolean"(必填), value: string|number|boolean(必填), description?: string, unit?: string }
- formulas: 数组，每项 { attributeId: string(必填, 引用 attributes 中某项的 name), expression: string(必填, mathjs 语法), description?: string }

## 参考示例（RPG 攻击力公式）
属性：attackBase(number, 10), level(number, 1), attackGrowth(number, 2)
公式：attack = attackBase + level * attackGrowth`,
    },
  ];
}
...
## apply_gdd 工具参数 schema
- sections: 数组，每项 { type: string(必填, 如 "overview"/"core_loop"/"mechanism"/"numeric"/"level"/"narrative"), title: string(必填), content: string(必填, Markdown) }`,
    },
  ];
}
...
## apply_loops 工具参数 schema
- loops: 数组，每项 { name: string(必填), description: string(必填), loopType: "core"|"secondary"|"meta"(必填), steps: [{ id: string, label: string, playerAction: string, emotion: string, order: number }](必填, 3-7 个) }

## 参考示例（核心循环）
name: "战斗成长循环", loopType: "core"
steps: [战斗→获得经验→升级→解锁技能→更强战斗]
`,
    },
  ];
}
...
## apply_moments 工具参数 schema
- moments: 数组，每项 { title: string(必填), description: string(必填), emotion: number(必填, 1-10 整数), emotionLabel: string, timing: number(0-100), type: "combat"|"exploration"|"puzzle"|"narrative"|"social"|"achievement"(必填), duration?: number }
`,
    },
  ];
}
...
## apply_rules 工具参数 schema
- rules: 数组，每项 { title: string(必填), condition: string(必填, IF 条件), action: string(必填, THEN 动作), category: "combat"|"economy"|"progression"|"social"|"system"|"content"(必填), priority: number(1-10 整数) }
`,
    },
  ];
}
...
## apply_level_flow 工具参数 schema
- name: string(必填)
- nodes: 数组，每项 { id: string, label: string(必填), type: "start"|"level"|"boss"|"secret"|"shop"|"cutscene"|"end"(必填), difficulty: number(1-10), duration?: number, description?: string }
- edges: 数组，每项 { source: string(节点 id), target: string(节点 id), type: "normal"|"secret"|"locked"|"branch"(必填), label?: string }
`,
    },
  ];
}
